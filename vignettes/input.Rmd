...
Each function returns a unique pointer of the relevant type (e.g., `std::unique_ptr<beachmat::logical_matrix>` for `lmat`).
...
while `in` can be an `iterator` of a `Rcpp::IntegerVector`, `Rcpp::LogicalVector` or `Rcpp::NumericVector` instance.

...
For specific matrix representations, special methods are available that can improve the efficiency of data access.
...
## From ordinary matrices

Ordinary R matrices containing double-precision numbers can be used to construct a `simple_numeric_matrix`, which provides a `get_const_col()` method.
This returns an iterator to a _Rcpp_ vector pointing to `first` row of column `c`.
The arguments are the same as `get_col()`^[Users can also specify `first` and `last`, though we will not do so here for simplicity.] except for the absence of an `iterator` argument.

```cpp
Rcpp::NumericVector::iterator it = dptr->get_const_col(c, /* size_t */);
```

For ordinary matrices, this method is more efficient than `get_col()` as it returns the iterator without needing to copy data into `work`.
The same method is also available for logical, integer and character inputs with the corresponding `simple_*_matrix` class. 
...
Column-sparse compressed matrices from `r CRANpkg("Matrix")` can be used to construct a `Csparse_*_matrix` class.
This has the `get_const_col_indexed()` method, which is guaranteed to index all of the non-zero entries in column `c` of the matrix.
...
Again, the iterators should be treated as read-only.

...
```cpp
matrix_type mat_type = dptr->get_matrix_type();
```
...
It is possible to directly support arbitrary user-supplied matrices, see `r Biocpkg("iSEE", vignette="external.html", label="here")` for more details.
...
The size of the blocks can be controlled using methods in the `r Biocpkg("DelayedArray")` package.

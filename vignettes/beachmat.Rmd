---
title: "beachmat: a C++ API for R matrix objects"
author: "Aaron Lun"
package: beachmat
output: 
  BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{beachmat: a C++ API for R matrix objects}
...
Returns the number of rows (`nrow`) in the matrix as a `size_t`.

```cpp
dptr->get_ncol();
```

Returns the number of columns (`ncol`) as a `size_t`.
...
```cpp
dptr->get_col(/* size_t */ c, 
              /* Rcpp::Vector::iterator */ in);
```

Takes an iterator `in` to a _Rcpp_ vector and fills it with values at column `c`.
...
Takes an iterator `in` to a _Rcpp_ vector and fills it with values at column `c` from row `first` to `last-1`.
...
Takes an iterator `in` to a _Rcpp_ vector and fills it with values at row `r`.
...
Takes an iterator `in` to a _Rcpp_ vector and fills it with values at row `r` from column `first` to `last-1`.
There should be at least `last-first` accessible elements, i.e., `*in` and `*(in+last-first-1)` should be valid entries.
No value is returned by this method.
...
Returns an iterator to a _Rcpp_ vector pointing to `first` row of column `c`.
...
```cpp
dptr->get_const_col_nonzero(/* size_t */ c, 
                            /* Rcpp::NumericVector::iterator */ work, 
                            /* size_t */ first, 
                            /* size_t */ last);
```

Returns a `const_col_nonzero_info` object, which is a tuple consisting of:

1. A `size_t`, specifying the number of non-zero entries in column `c` from rows `[first, last)`.
2. A `Rcpp::IntegerVector::iterator`, pointing to a vector containing the row index for each non-zero entry in column `c` from rows `[first, last)`.
3. A `Rcpp::NumericVector::iterator`, pointing to a vector containing the values of each non-zero entry in column `c` from rows `[first, last)`.
...
This method is quite efficient for `dgCMatrix` objects, as it can directly return iterators to the indices and values of the non-zero entries.
...
Returns a unique pointer to a `numeric_matrix` instance of the same type as that pointed to by `dptr`.

```
dptr->get_matrix_type()
```

Returns a `matrix_type` value specifying the specific matrix representation that is pointed to by `dptr`.
...
Returns a `Rcpp::RObject` containing the original R matrix that was used to create `dptr`.

...
test_that("Output mode choices are okay", {
    # This manually checks that the output class choices in beachmat::output_param are correct.
    ...
    expect_identical(check_output_mode("unknown", simplify=TRUE, preserve.zero=FALSE), "simple")
    expect_identical(check_output_mode("unknown", simplify=FALSE, preserve.zero=FALSE), "HDF5")
    expect_identical(check_output_mode("unknown", simplify=TRUE, preserve.zero=TRUE), "simple")
    expect_identical(check_output_mode("unknown", simplify=FALSE, preserve.zero=TRUE), "HDF5")
})
